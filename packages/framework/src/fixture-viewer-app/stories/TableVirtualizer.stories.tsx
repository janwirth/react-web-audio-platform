...
import { TableVirtualizer as TableVirtualizerComponent, TableVirtualizerHandle } from '@/components/TableVirtualizer';
...
// Generate items with alphabetical titles for sorting/indexing demo
const generateAlphabeticalItems = () => {
  const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const items: Array<{ id: number; title: string; description: string; category: string; status: string }> = [];
  
  letters.split('').forEach((letter, letterIndex) => {
    // Generate 5-15 items per letter
    const count = 5 + Math.floor(Math.random() * 11);
    for (let i = 0; i < count; i++) {
      items.push({
        id: items.length,
        title: `${letter}${letterIndex * 100 + i + 1}`,
        description: `Item starting with ${letter}, number ${i + 1}`,
...
export const WithAlphabeticalIndices: Story = {
  render: TableWithAlphabeticalIndices,
};
...
// Component for scrollbar overlay with alphabetical indices
function AlphabeticalScrollbarOverlay({
  items,
  itemHeight,
  scrollTop,
  containerHeight,
  ...
  // Calculate max scrollTop: we can scroll so the last item is at the top
  const maxScrollTop = Math.max(0, (items.length - 1) * itemHeight);
  // Total scrollable size includes the overscroll padding
  const scrollableSize = maxScrollTop + containerHeight;
  
  // Create index map: letter -> first index (uneven distribution)
  ...
  // Calculate letter positions on scrollbar (uneven distribution based on actual item positions)
  const letterPositions = useMemo(() => {
    const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('');
    const positions: Array<{ letter: string; position: number; index: number }> = [];
    
    letters.forEach((letter) => {
      const index = letterIndices[letter];
      if (index !== undefined) {
        // Calculate scroll position for this letter's first item
        const letterScrollTop = index * itemHeight;
        // Map to scrollbar track position (0 to containerHeight)
        // The scrollbar track represents the scrollable area
        const position = scrollableSize > 0
          ? (letterScrollTop / scrollableSize) * containerHeight
          : 0;
        positions.push({ letter, position, index });
      }
    });
    
    return positions;
  }, [letterIndices, itemHeight, scrollableSize, containerHeight]);

  if (scrollableSize <= 0 || containerHeight <= 0) return null;
  ...
  return (
    ...
    >
      {letterPositions.map(({ letter, position, index }) => {
        // Clamp position to visible scrollbar area
        const clampedPosition = Math.max(2, Math.min(containerHeight - 14, position));
        return (
          <button
            key={letter}
            onClick={(e) => {
...
// Story with alphabetical sidebar indices
function TableWithAlphabeticalIndices() {
  const [, setDebugView] = useAtom(debugViewAtom);
  const [items] = useState(() => generateAlphabeticalItems());
  ...
  useEffect(() => {
    const handleScroll = () => {
      const visibleRange = virtualizerRef.current?.getVisibleRange();
      if (visibleRange) {
        setScrollTop(visibleRange.start * itemHeight);
      }
    };
  ...
  useEffect(() => {
    const measure = () => {
      if (containerRef.current) {
        const scrollable = containerRef.current.querySelector('[tabindex="0"]') as HTMLElement;
        if (scrollable) {
          setContainerHeight(scrollable.clientHeight);
        }
      }
    };
    measure();
    ...
    if (containerRef.current) {
      const scrollable = containerRef.current.querySelector('[tabindex="0"]');
      if (scrollable) resizeObserver.observe(scrollable);
    }
    return () => resizeObserver.disconnect();
  }, []);

  // Create index map: letter -> first index
  const letterIndices = useMemo(() => {
    const map: Record<string, number> = {};
    items.forEach((item, index) => {
      const firstLetter = item.title[0].toUpperCase();
    ...
    <Column className="h-full w-full" style={{ height: '100vh' }}>
      ...
      <Row className="flex-1" style={{ minHeight: 0 }}>
        ...
        >
          ...
          >
            TableVirtualizer with Alphabetical Indices (on Scrollbar)
          </div>
          <div
            ref={containerRef}
            style={{
          ...
          >
            <TableVirtualizerComponent
              ...
              renderItem={(item, index) => {
                const firstLetter = item.title[0].toUpperCase();
                const showLetter = index === 0 || items[index - 1].title[0].toUpperCase() !== firstLetter;
                  ...
                  <div
                    style={{
                      ...
                      backgroundColor: isSelected
                        ...
                        : 'transparent',
                      borderLeft: isSelected ? '3px solid currentColor' : 'none',
                    }}
                  >
                    {isSelected && (
                      <div
...
function TableWithAllFeatures() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const visibleRange = virtualizerRef.current?.getVisibleRange();
      if (visibleRange) {
        setScrollTop(visibleRange.start * 60); // itemHeight is 60
      }
    };
            ...
            <Column style={{ flex: 1, minHeight: 0 }}>
              <TableVirtualizerComponent
                ...
                renderItem={(item, index) => {
                  const isSelected = index === cursorIndex;
                  const firstLetter = item.title[0].toUpperCase();
                  const showLetter = index === 0 || sortedItems[index - 1].title[0].toUpperCase() !== firstLetter;
                  ...
                  return (
                    ...
                    >
                      ...
                      {!showLetter && <div style={{ width: '20px', marginRight: '8px' }} />}
                      <div style={{ flex: '0 0 60px' }}>#{item.id}</div>
                      <div
                        style={{
                          flex: 1,
